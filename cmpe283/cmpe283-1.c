...
/*
 * Pinbased VM-Execution  capabilities
 * See SDM volume 3C, section 23.6.1, Table 23-5
 */
...
/*
 * Primary processor based VM-Execution capabilities
 * See SDM volume 3C, section 23.6.2, Table 23-6
 */
...
/*
 * Secondary processor based VM-Execution capabilities
 * See SDM volume 3C, section 23.6.2, Table 23-7
 */
...
/*
 * VM-Exit Controls
 * SDM volume 3C, section 23.7.1 Table 23-12
 */

struct capability_info exit_ctrl[14] =
{
...
/*
 * VM-Entry Controls
 * SDM volume 3C, section 23.8.1 Table 23-14
 */
struct capability_info entry_ctrl[12] =
{
...
void
check_availability_secondary_procbased_ctrl(void)
{
	uint32_t lo, hi;

	/* Procbased controls */
	rdmsr(MSR_IA32_VMX_PROCBASED_CTLS, lo, hi);

	if ( hi & (1 << (63 - 32)))
	{
	printk("Secondary procbased controls are available");
		/* Secondary Procbased Controls */
		rdmsr(MSR_IA32_VMX_PROCBASED_CTLS2, lo, hi);
...
detect_vmx_features(void)
{
	...
	/* Pinbased controls */
	rdmsr(MSR_IA32_VMX_PINBASED_CTLS, lo, hi);
	...
	check_availability_secondary_procbased_ctrl();
        /*rdmsr(MSR_IA32_VMX_PROCBASED_CTLS2, lo, hi);
        pr_info("Secondary Procbased Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(secondary_procbased, 27, lo, hi);*/
	...
	/* VM_Exit controls */
        rdmsr(MSR_IA32_VMX_EXIT_CTLS, lo, hi);
        pr_info("Exit Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(exit_ctrl, 14, lo, hi);

	/* VM-Entry controls */
        rdmsr(MSR_IA32_VMX_ENTRY_CTLS, lo, hi);
        pr_info("Entry Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(entry_ctrl, 12, lo, hi);
